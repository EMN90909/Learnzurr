...
if (fs.existsSync(wizardPath)) {
  ...
  src = src.replace(/<div className="rounded-2xl border border-\[var\(--clay-border\)\][\s\S]*?<li>✓ 1 number<\/li><\/ul><\/div>/g, "");
  ...
  src = src.replace(/<div className="flex justify-center rounded-2xl[\s\S]*?data-expired-callback="onStrutaCaptchaExpired"><\/div><\/div>/g, "");
  src = src.replace(/<div className="rounded-2xl border border-\[var\(--clay-border\)\][\s\S]*?data-callback="onStrutaCaptcha"><\/div><\/div>/g, "");
  src = src.replace(/\n\s*const \[captchaToken, setCaptchaToken\] = useState\(""\);/g, "");
...
if (fs.existsSync(pkgPath)) {
  ...
  for (const key of ["prebuild", "prestart", "predev"]) {
    if (pkg.scripts?.[key] && !pkg.scripts[key].includes("hcaptcha-otp-setup-fixes.mjs")) pkg.scripts[key] += " && node scripts/hcaptcha-otp-setup-fixes.mjs";
  }
