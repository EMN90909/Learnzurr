...
func buildAudit(action, actorID, subjectID, severity string, metadata map[string]string) AuditEvent {
    return AuditEvent{Engine:"gamfy", Action:action, ActorID:actorID, SubjectID:subjectID, Severity:severity, Metadata:metadata, CreatedAt:time.Now().UTC()}
}
...
func enqueueJob(action string, actor Actor, payload map[string]string) JobEnvelope {
    return JobEnvelope{
        ID: stableID("gamfy", action, actor.ID, fmt.Sprint(time.Now().UnixNano())),
        Type: "gamfy." + action,
        ActorID: actor.ID,
        Engine: "gamfy",
        Payload: payload,
...
func cacheKey(parts ...string) string { return "learnzur:gamfy:" + strings.Join(parts, ":") }


// AwardPointsRequest contains the validated command payload used by the gamfy engine.
...
func ValidateAwardPoints(req AwardPointsRequest) []ValidationIssue {
...
func HandleAwardPoints(req AwardPointsRequest) (AwardPointsResponse, error) {
    if issues := ValidateAwardPoints(req); len(issues) > 0 {
        return AwardPointsResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return AwardPointsResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("AwardPoints", req.Actor, cleaned)
    audit := buildAudit("AwardPoints", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("AwardPoints", req.SubjectID)})
    return AwardPointsResponse{OK:true, Message:"AwardPoints accepted", CacheKey:cacheKey("AwardPoints", req.SubjectID), Job:job, Audit:audit}, nil
}

// UnlockBadgeRequest contains the validated command payload used by the gamfy engine.
...
type UnlockBadgeResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateUnlockBadge(req UnlockBadgeRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleUnlockBadge(req UnlockBadgeRequest) (UnlockBadgeResponse, error) {
    if issues := ValidateUnlockBadge(req); len(issues) > 0 {
        return UnlockBadgeResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return UnlockBadgeResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("UnlockBadge", req.Actor, cleaned)
    audit := buildAudit("UnlockBadge", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("UnlockBadge", req.SubjectID)})
    return UnlockBadgeResponse{OK:true, Message:"UnlockBadge accepted", CacheKey:cacheKey("UnlockBadge", req.SubjectID), Job:job, Audit:audit}, nil
}

// UpdateStreakRequest contains the validated command payload used by the gamfy engine.
...
func ValidateUpdateStreak(req UpdateStreakRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleUpdateStreak(req UpdateStreakRequest) (UpdateStreakResponse, error) {
    if issues := ValidateUpdateStreak(req); len(issues) > 0 {
        return UpdateStreakResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return UpdateStreakResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("UpdateStreak", req.Actor, cleaned)
    audit := buildAudit("UpdateStreak", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("UpdateStreak", req.SubjectID)})
    return UpdateStreakResponse{OK:true, Message:"UpdateStreak accepted", CacheKey:cacheKey("UpdateStreak", req.SubjectID), Job:job, Audit:audit}, nil
}

// GetLeaderboardRequest contains the validated command payload used by the gamfy engine.
...
func ValidateGetLeaderboard(req GetLeaderboardRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleGetLeaderboard(req GetLeaderboardRequest) (GetLeaderboardResponse, error) {
    if issues := ValidateGetLeaderboard(req); len(issues) > 0 {
        return GetLeaderboardResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return GetLeaderboardResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("GetLeaderboard", req.Actor, cleaned)
    audit := buildAudit("GetLeaderboard", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("GetLeaderboard", req.SubjectID)})
    return GetLeaderboardResponse{OK:true, Message:"GetLeaderboard accepted", CacheKey:cacheKey("GetLeaderboard", req.SubjectID), Job:job, Audit:audit}, nil
}

// GetAgeAdaptiveConfigRequest contains the validated command payload used by the gamfy engine.
type GetAgeAdaptiveConfigRequest struct {
    Actor Actor
    SubjectID string
    Fields map[string]string
    IdempotencyKey string
...
type GetAgeAdaptiveConfigResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateGetAgeAdaptiveConfig(req GetAgeAdaptiveConfigRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleGetAgeAdaptiveConfig(req GetAgeAdaptiveConfigRequest) (GetAgeAdaptiveConfigResponse, error) {
    if issues := ValidateGetAgeAdaptiveConfig(req); len(issues) > 0 {
        return GetAgeAdaptiveConfigResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return GetAgeAdaptiveConfigResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("GetAgeAdaptiveConfig", req.Actor, cleaned)
    audit := buildAudit("GetAgeAdaptiveConfig", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("GetAgeAdaptiveConfig", req.SubjectID)})
    return GetAgeAdaptiveConfigResponse{OK:true, Message:"GetAgeAdaptiveConfig accepted", CacheKey:cacheKey("GetAgeAdaptiveConfig", req.SubjectID), Job:job, Audit:audit}, nil
}
