...
func buildAudit(action, actorID, subjectID, severity string, metadata map[string]string) AuditEvent {
    return AuditEvent{Engine:"find", Action:action, ActorID:actorID, SubjectID:subjectID, Severity:severity, Metadata:metadata, CreatedAt:time.Now().UTC()}
}
...
func enqueueJob(action string, actor Actor, payload map[string]string) JobEnvelope {
    return JobEnvelope{
        ID: stableID("find", action, actor.ID, fmt.Sprint(time.Now().UnixNano())),
        Type: "find." + action,
        ActorID: actor.ID,
        Engine: "find",
        Payload: payload,
...
func cacheKey(parts ...string) string { return "learnzur:find:" + strings.Join(parts, ":") }


// SearchRequest contains the validated command payload used by the find engine.
...
func ValidateSearch(req SearchRequest) []ValidationIssue {
...
func HandleSearch(req SearchRequest) (SearchResponse, error) {
    if issues := ValidateSearch(req); len(issues) > 0 {
        return SearchResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return SearchResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("Search", req.Actor, cleaned)
    audit := buildAudit("Search", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("Search", req.SubjectID)})
    return SearchResponse{OK:true, Message:"Search accepted", CacheKey:cacheKey("Search", req.SubjectID), Job:job, Audit:audit}, nil
}

// GetSSRClassPageRequest contains the validated command payload used by the find engine.
type GetSSRClassPageRequest struct {
    Actor Actor
    SubjectID string
    Fields map[string]string
    IdempotencyKey string
...
type GetSSRClassPageResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateGetSSRClassPage(req GetSSRClassPageRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleGetSSRClassPage(req GetSSRClassPageRequest) (GetSSRClassPageResponse, error) {
    if issues := ValidateGetSSRClassPage(req); len(issues) > 0 {
        return GetSSRClassPageResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return GetSSRClassPageResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("GetSSRClassPage", req.Actor, cleaned)
    audit := buildAudit("GetSSRClassPage", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("GetSSRClassPage", req.SubjectID)})
    return GetSSRClassPageResponse{OK:true, Message:"GetSSRClassPage accepted", CacheKey:cacheKey("GetSSRClassPage", req.SubjectID), Job:job, Audit:audit}, nil
}

// GetSearchSuggestionsRequest contains the validated command payload used by the find engine.
...
func ValidateGetSearchSuggestions(req GetSearchSuggestionsRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleGetSearchSuggestions(req GetSearchSuggestionsRequest) (GetSearchSuggestionsResponse, error) {
    if issues := ValidateGetSearchSuggestions(req); len(issues) > 0 {
        return GetSearchSuggestionsResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return GetSearchSuggestionsResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("GetSearchSuggestions", req.Actor, cleaned)
    audit := buildAudit("GetSearchSuggestions", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("GetSearchSuggestions", req.SubjectID)})
    return GetSearchSuggestionsResponse{OK:true, Message:"GetSearchSuggestions accepted", CacheKey:cacheKey("GetSearchSuggestions", req.SubjectID), Job:job, Audit:audit}, nil
}

// LogSearchQueryRequest contains the validated command payload used by the find engine.
...
func ValidateLogSearchQuery(req LogSearchQueryRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleLogSearchQuery(req LogSearchQueryRequest) (LogSearchQueryResponse, error) {
    if issues := ValidateLogSearchQuery(req); len(issues) > 0 {
        return LogSearchQueryResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return LogSearchQueryResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("LogSearchQuery", req.Actor, cleaned)
    audit := buildAudit("LogSearchQuery", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("LogSearchQuery", req.SubjectID)})
    return LogSearchQueryResponse{OK:true, Message:"LogSearchQuery accepted", CacheKey:cacheKey("LogSearchQuery", req.SubjectID), Job:job, Audit:audit}, nil
}

// GetTrendingSubjectsRequest contains the validated command payload used by the find engine.
type GetTrendingSubjectsRequest struct {
    Actor Actor
    SubjectID string
    Fields map[string]string
    IdempotencyKey string
...
type GetTrendingSubjectsResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateGetTrendingSubjects(req GetTrendingSubjectsRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleGetTrendingSubjects(req GetTrendingSubjectsRequest) (GetTrendingSubjectsResponse, error) {
    if issues := ValidateGetTrendingSubjects(req); len(issues) > 0 {
        return GetTrendingSubjectsResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return GetTrendingSubjectsResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("GetTrendingSubjects", req.Actor, cleaned)
    audit := buildAudit("GetTrendingSubjects", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("GetTrendingSubjects", req.SubjectID)})
    return GetTrendingSubjectsResponse{OK:true, Message:"GetTrendingSubjects accepted", CacheKey:cacheKey("GetTrendingSubjects", req.SubjectID), Job:job, Audit:audit}, nil
}
