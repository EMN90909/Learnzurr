...
func buildAudit(action, actorID, subjectID, severity string, metadata map[string]string) AuditEvent {
    return AuditEvent{Engine:"classroom", Action:action, ActorID:actorID, SubjectID:subjectID, Severity:severity, Metadata:metadata, CreatedAt:time.Now().UTC()}
}
...
func enqueueJob(action string, actor Actor, payload map[string]string) JobEnvelope {
    return JobEnvelope{
        ID: stableID("classroom", action, actor.ID, fmt.Sprint(time.Now().UnixNano())),
        Type: "classroom." + action,
        ActorID: actor.ID,
        Engine: "classroom",
        Payload: payload,
        Attempts: 0,
...
func cacheKey(parts ...string) string { return "learnzur:classroom:" + strings.Join(parts, ":") }


// CreateRoomRequest contains the validated command payload used by the classroom engine.
...
func ValidateCreateRoom(req CreateRoomRequest) []ValidationIssue {
...
func HandleCreateRoom(req CreateRoomRequest) (CreateRoomResponse, error) {
    if issues := ValidateCreateRoom(req); len(issues) > 0 {
        return CreateRoomResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return CreateRoomResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("CreateRoom", req.Actor, cleaned)
    audit := buildAudit("CreateRoom", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("CreateRoom", req.SubjectID)})
    return CreateRoomResponse{OK:true, Message:"CreateRoom accepted", CacheKey:cacheKey("CreateRoom", req.SubjectID), Job:job, Audit:audit}, nil
}

// JoinRoomRequest contains the validated command payload used by the classroom engine.
...
func ValidateJoinRoom(req JoinRoomRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleJoinRoom(req JoinRoomRequest) (JoinRoomResponse, error) {
    if issues := ValidateJoinRoom(req); len(issues) > 0 {
        return JoinRoomResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return JoinRoomResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("JoinRoom", req.Actor, cleaned)
    audit := buildAudit("JoinRoom", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("JoinRoom", req.SubjectID)})
    return JoinRoomResponse{OK:true, Message:"JoinRoom accepted", CacheKey:cacheKey("JoinRoom", req.SubjectID), Job:job, Audit:audit}, nil
}

// ToggleCameraRequest contains the validated command payload used by the classroom engine.
...
func ValidateToggleCamera(req ToggleCameraRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleToggleCamera(req ToggleCameraRequest) (ToggleCameraResponse, error) {
    if issues := ValidateToggleCamera(req); len(issues) > 0 {
        return ToggleCameraResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return ToggleCameraResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("ToggleCamera", req.Actor, cleaned)
    audit := buildAudit("ToggleCamera", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("ToggleCamera", req.SubjectID)})
    return ToggleCameraResponse{OK:true, Message:"ToggleCamera accepted", CacheKey:cacheKey("ToggleCamera", req.SubjectID), Job:job, Audit:audit}, nil
}

// HandleDisconnectRequest contains the validated command payload used by the classroom engine.
type HandleDisconnectRequest struct {
    Actor Actor
    SubjectID string
    Fields map[string]string
    IdempotencyKey string
...
type HandleDisconnectResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateHandleDisconnect(req HandleDisconnectRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleHandleDisconnect(req HandleDisconnectRequest) (HandleDisconnectResponse, error) {
    if issues := ValidateHandleDisconnect(req); len(issues) > 0 {
        return HandleDisconnectResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return HandleDisconnectResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("HandleDisconnect", req.Actor, cleaned)
    audit := buildAudit("HandleDisconnect", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("HandleDisconnect", req.SubjectID)})
    return HandleDisconnectResponse{OK:true, Message:"HandleDisconnect accepted", CacheKey:cacheKey("HandleDisconnect", req.SubjectID), Job:job, Audit:audit}, nil
}

// HandleReconnectRequest contains the validated command payload used by the classroom engine.
type HandleReconnectRequest struct {
    Actor Actor
    SubjectID string
    Fields map[string]string
    IdempotencyKey string
...
type HandleReconnectResponse struct {
    OK bool
    Message string
    CacheKey string
    Job JobEnvelope
...
func ValidateHandleReconnect(req HandleReconnectRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleHandleReconnect(req HandleReconnectRequest) (HandleReconnectResponse, error) {
    if issues := ValidateHandleReconnect(req); len(issues) > 0 {
        return HandleReconnectResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return HandleReconnectResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("HandleReconnect", req.Actor, cleaned)
    audit := buildAudit("HandleReconnect", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("HandleReconnect", req.SubjectID)})
    return HandleReconnectResponse{OK:true, Message:"HandleReconnect accepted", CacheKey:cacheKey("HandleReconnect", req.SubjectID), Job:job, Audit:audit}, nil
}

// EndRoomRequest contains the validated command payload used by the classroom engine.
...
func ValidateEndRoom(req EndRoomRequest) []ValidationIssue {
    issues := validateRequired(map[string]string{"actor_id": req.Actor.ID, "subject_id": req.SubjectID})
    if req.IdempotencyKey == "" { issues = append(issues, ValidationIssue{Field:"idempotency_key", Code:"required", Message:"idempotency key prevents duplicate actions"}) }
    for k, v := range req.Fields {
        if strings.Contains(strings.ToLower(v), "<script") { issues = append(issues, ValidationIssue{Field:k, Code:"unsafe_html", Message:"script tags are not allowed"}) }
...
func HandleEndRoom(req EndRoomRequest) (EndRoomResponse, error) {
    if issues := ValidateEndRoom(req); len(issues) > 0 {
        return EndRoomResponse{OK:false, Message:"validation failed", Issues:issues}, ErrValidation
    }
    cleaned := map[string]string{}
    for k, v := range req.Fields { cleaned[k] = sanitizeText(v, 5000) }
    decision := requireRole(req.Actor, "admin", "teacher", "parent", "learner")
    if !decision.Allowed {
        return EndRoomResponse{OK:false, Message:decision.Reason}, ErrForbidden
    }
    job := enqueueJob("EndRoom", req.Actor, cleaned)
    audit := buildAudit("EndRoom", req.Actor.ID, req.SubjectID, "info", map[string]string{"idempotency_key":req.IdempotencyKey, "cache_key":cacheKey("EndRoom", req.SubjectID)})
    return EndRoomResponse{OK:true, Message:"EndRoom accepted", CacheKey:cacheKey("EndRoom", req.SubjectID), Job:job, Audit:audit}, nil
}
