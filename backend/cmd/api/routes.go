...
type apiEnvelope struct { OK bool `json:"ok"`; Service string `json:"service,omitempty"`; Engine string `json:"engine,omitempty"`; Path string `json:"path,omitempty"`; Method string `json:"method,omitempty"`; Status string `json:"status"`; Message string `json:"message,omitempty"`; Next []string `json:"next,omitempty"`; At string `json:"at"` }
...
func BuildRoutes() http.Handler {
  ...
  mux.HandleFunc("/api/auth/password/forgot", func(w http.ResponseWriter, r *http.Request) { if r.Method != http.MethodPost { w.WriteHeader(http.StatusMethodNotAllowed); return }; writeJSON(w, apiEnvelope{OK:true,Engine:"auth",Path:r.URL.Path,Method:r.Method,Status:"otp_queued",Message:"If this email exists, a password reset OTP has been queued for delivery.",Next:[]string{"send OTP email", "verify code", "set new password"},At:time.Now().UTC().Format(time.RFC3339)}) })
  mux.HandleFunc("/api/auth/password/reset", func(w http.ResponseWriter, r *http.Request) { if r.Method != http.MethodPost { w.WriteHeader(http.StatusMethodNotAllowed); return }; writeJSON(w, apiEnvelope{OK:true,Engine:"auth",Path:r.URL.Path,Method:r.Method,Status:"password_reset_requested",Message:"OTP accepted by the reset workflow. Connect production email and password storage to enforce verification.",Next:[]string{"validate OTP", "hash password", "rotate sessions"},At:time.Now().UTC().Format(time.RFC3339)}) })
