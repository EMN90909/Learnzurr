-- 226_production_contract_226.sql
-- Production contract migration for Learnzur using Supabase PostgreSQL.

create table if not exists public.production_contract_226 (
...
insert into public.production_contract_226 (role, workflow, api_path, required_tables, redis_stream, audit_table, risk_level) values
...
create index if not exists idx_production_contract_226_role_active on public.production_contract_226(role, active, risk_level desc);
comment on table public.production_contract_226 is 'Supabase workflow contract table used to validate Learnzur API, Redis Stream and audit wiring.';
