...
type AdminBranchesProps = {
  initialRoleFilter?: "all" | "family" | "operations" | "marketplace" | "admin";
  title?: string;
  description?: string;
};
...
const displayName = (user: any) => user.home_name || user.business_name || user.company_name || user.full_name || user.name || user.email || "Unnamed";

const normalizeUser = (user: any) => {
  const role = normalizeRole(user.role || user.user_role || user.profile?.role);
  const subscription = user.subscription || user.current_subscription || {};
  const planCode = user.plan_code || user.plan || subscription.plan_code || subscription.plan_name || (user.is_pro ? "pro" : "free");
  const rawPlanStatus = String(user.plan_status || subscription.status || user.subscription_status || (user.is_pro ? "paid" : "free")).toLowerCase();
  const banned = Boolean(user.is_banned || user.banned || user.active === false || user.status === "banned");
  const paid = Boolean(user.is_pro || ["paid", "active", "pro", "trialing"].includes(rawPlanStatus) || !["", "free", "cancelled", "canceled"].includes(String(planCode || "free").toLowerCase()));
  return {
    ...user,
    role,
    plan_code: planCode || "free",
    plan_status: banned ? "restricted" : paid ? "paid" : "free",
    isPro: paid && !banned,
    isBanned: banned,
  };
};
...
const AdminBranches = ({
  ...
  title = "User & Provider Directory",
  description = "Manage registered families, funeral homes, vendors, and platform team accounts.",
}: AdminBranchesProps) => {
  ...
  const loadUsers = async () => {
    setLoading(true);
    try {
      const response = await apiFetch("/api/admin/users?perPage=100");
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || "Could not load users directory.");
      setUsers((result.users || result.data || []).map(normalizeUser));
    } catch (error: any) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      const matchesSearch = !q || `${displayName(user)} ${user.email || ""} ${user.phone || ""} ${user.county || ""} ${user.town || ""} ${user.role || ""} ${user.plan_status || ""}`.toLowerCase().includes(q);
      ...
      <div className="max-w-7xl mx-auto space-y-8 bg-[var(--paper)] px-2 md:px-0">
        ...
        <Card className="rounded-none border-[var(--border)] bg-[var(--surface)] shadow-none">
          <CardHeader className="border-b border-[var(--border)]">
            ...
            <CardDescription>Supabase Auth users joined with profile and plan fields from the admin API.</CardDescription>
          </CardHeader>
