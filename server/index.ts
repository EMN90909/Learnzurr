...
if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
  webpush.setVapidDetails(process.env.VAPID_SUBJECT ?? "mailto:support@learnzurr.com", process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
}
...
const inviteSchema = z.object({ email: z.string().email(), percentage: z.number().min(0).max(100), teamId: z.string().uuid(), inviterName: z.string().max(120).optional() });
const sessionSchema = z.object({ name: z.string().trim().min(3).max(120), startsAt: z.string().datetime(), endsAt: z.string().datetime() });
const assignmentSchema = z.object({ title: z.string().trim().min(3).max(160), body: z.record(z.unknown()), kind: z.enum(["task", "question", "quiz", "assessment", "exam"]), dueAt: z.string().datetime().nullable().optional() });
const paymentSchema = z.object({ email: z.string().email(), amount: z.number().min(10), currency: z.string().length(3).default("KES"), classId: z.string().uuid().optional(), studentId: z.string().uuid().optional() });
const pushSchema = z.object({ endpoint: z.string().url(), keys: z.object({ p256dh: z.string(), auth: z.string() }) });
...
async function classForTeacher(userId: string, classId: string) {
  ...
  const { data: mapping } = await admin!.from("class_teachers").select("class_id").eq("class_id", classId).eq("teacher_id", userId).maybeSingle();
  if (!mapping) throw Object.assign(new Error("You are not assigned to this class"), { statusCode: 403 });
...
app.get("/api/health", (_req, res) => res.json({ ok: true, api: "typescript-express", auth: Boolean(admin), payments: Boolean(paystackKey), smtp: Boolean(smtp), webPush: Boolean(process.env.VAPID_PUBLIC_KEY), signaling: signalingUrl }));

app.get("/api/teacher/dashboard", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    await teacherProfile(req.user!.id);
    const { data: owned } = await admin!.from("classes").select("id,title").eq("owner_teacher_id", req.user!.id);
    const { data: mapped } = await admin!.from("class_teachers").select("classes(id,title)").eq("teacher_id", req.user!.id);
    const classes = [...(owned ?? []), ...(mapped ?? []).flatMap((row: any) => row.classes ? [row.classes] : [])];
    const unique = [...new Map(classes.map((item: any) => [item.id, item])).values()] as { id: string; title: string }[];
    const ids = unique.map((item) => item.id);
    const studentsResult = ids.length ? await admin!.from("student_enrollments").select("student_id", { count: "exact", head: true }).in("class_id", ids).eq("active", true) : { count: 0 };
    const paymentsResult = ids.length ? await admin!.from("payments").select("amount_kes,status").in("class_id", ids) : { data: [] as any[] };
    const { data: teams } = await admin!.from("teacher_teams").select("id,name").eq("owner_id", req.user!.id);
    const teamIds = (teams ?? []).map((team) => team.id);
    const membersResult = teamIds.length ? await admin!.from("team_members").select("teacher_id,revenue_share").in("team_id", teamIds) : { data: [] as any[] };
    const revenue = (paymentsResult.data ?? []).filter((payment: any) => payment.status === "paid").reduce((sum: number, payment: any) => sum + Number(payment.amount_kes), 0);
    return res.json({ students: studentsResult.count ?? 0, revenue, classes: unique, teams: teams ?? [], teachers: membersResult.data ?? [] });
  } catch (error) { next(error); }
});
...
app.get("/api/team/:teamId", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const membership = await admin!.from("team_members").select("teacher_id").eq("team_id", teamId).eq("teacher_id", req.user!.id).maybeSingle();
    if (team.owner_id !== req.user!.id && !membership.data) return res.status(403).json({ error: "Team access denied" });
    const { data: members } = await admin!.from("team_members").select("teacher_id,revenue_share,last_report_at,profiles!team_members_teacher_id_fkey(full_name,avatar_url,last_login_at)").eq("team_id", teamId);
...
app.post("/api/team/invite", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    const profile = await teacherProfile(req.user!.id);
    ...
    await admin!.from("teacher_invites").upsert({ team_id: input.teamId, email: input.email.toLowerCase(), revenue_share: input.percentage, invited_by: req.user!.id, invited_user_id: data.user.id, status: "pending" }, { onConflict: "team_id,email" });
    return res.status(201).json({ invited: true, message: `A secure signup link was sent to ${input.email} through Supabase SMTP.` });
  } catch (error) { next(error); }
});
...
app.post("/api/push/subscribe", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const { error } = await admin!.from("push_subscriptions").upsert({ user_id: req.user!.id, endpoint: input.endpoint, p256dh: input.keys.p256dh, auth: input.keys.auth }, { onConflict: "endpoint" });
...
app.post("/api/classes/:classId/sessions", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const classRecord = await classForTeacher(req.user!.id, classId);
    ...
    const room = `class-${classId}-${crypto.randomBytes(8).toString("hex")}`;
    const { data: session, error } = await admin!.from("live_sessions").insert({ class_id: classId, name: input.name, starts_at: input.startsAt, ends_at: input.endsAt, join_token_hash: crypto.createHash("sha256").update(rawToken).digest("hex"), signaling_room: room, created_by: req.user!.id }).select("id,name,starts_at,ends_at,signaling_room").single();
    if (error || !session) throw error ?? new Error("Session could not be created");
    const { data: enrollments } = await admin!.from("student_enrollments").select("student_id").eq("class_id", classId).eq("active", true);
    ...
    const teacherName = profile.full_name || req.user!.email || "Your teacher";
    let emailsSent = 0;
    if (smtp && learnerIds.length) {
      ...
      if (emails.length) {
        await smtp.sendMail({ from: process.env.SMTP_FROM ?? "Learnzurr <no-reply@learnzurr.com>", bcc: emails, subject: `${input.name} starts ${new Date(input.startsAt).toLocaleString()}`, text: `${teacherName} scheduled ${input.name} for ${new Date(input.startsAt).toLocaleString()}. Sign in and join: ${joinUrl}`, html: `<h2>${input.name}</h2><p>${teacherName} scheduled this class for <strong>${new Date(input.startsAt).toLocaleString()}</strong>.</p><p><a href="${joinUrl}">Sign in and join class</a></p>` });
    ...
    return res.status(201).json({ session: { ...session, joinUrl, signalingUrl, classTitle: classRecord.title }, notified: { learners: learnerIds.length, emailsSent, pushesSent } });
  } catch (error) { next(error); }
});
...
app.post("/api/classes/:classId/assignments", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    await classForTeacher(req.user!.id, classId);
    const { data, error } = await admin!.from("assignments").insert({ class_id: classId, author_id: req.user!.id, title: input.title, body: input.body, kind: input.kind, due_at: input.dueAt ?? null }).select("id,title,kind,due_at").single();
    if (error) throw error;
...
app.post("/api/live/:sessionId/join", requireUser, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const role = profile?.role as "teacher" | "learner";
    let permitted = false;
    ...
    if (role === "teacher") permitted = Boolean((await admin!.from("classes").select("id").eq("id", session.class_id).eq("owner_teacher_id", req.user!.id).maybeSingle()).data) || Boolean((await admin!.from("class_teachers").select("teacher_id").eq("class_id", session.class_id).eq("teacher_id", req.user!.id).maybeSingle()).data);
    ...
    const socketToken = crypto.createHmac("sha256", signalingSecret).update(`${session.id}:${req.user!.id}:${role}`).digest("base64url");
