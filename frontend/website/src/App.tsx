...
function AuthPage({ mode, role }: { mode: "signin" | "signup"; role?: AppRole }) {
  ...
  const [form, setForm] = useState({ name: "", email: search.get("email") ?? "", password: "", phone: "", school: "", childName: "", organisation: "" });
  ...
  async function submit(event: FormEvent) {
    ...
    const metadata = {
      role, full_name: form.name, phone: form.phone, school: form.school, child_name: form.childName,
      organisation: form.organisation, team_id: search.get("team") ?? undefined,
    };
  ...
  return <div className="auth-shell"><section className="auth-copy"><a className="brand" href="/"><span>L</span>Learnzurr</a><h1>{mode === "signin" ? "Welcome back" : `Create your ${role} account`}</h1><p>{mode === "signin" ? "One secure sign-in for teachers, learners, guardians, and administrators." : "This form is tailored to your role. Email verification is required before dashboard access."}</p></section><form className="auth-card" onSubmit={submit}>
    {mode === "signup" && <label>Full name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })}/></label>}
    <label>Email<input required type="email" readOnly={Boolean(search.get("email"))} value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })}/></label>
    {mode === "signup" && role === "teacher" && <label>School or team<input value={form.school} onChange={(event) => setForm({ ...form, school: event.target.value })}/></label>}
...
function RolePage({ role, slug }: { role: AppRole; slug: string }) {
  ...
  if (role === "teacher" && slug === "team") return <TeacherTeam teamId={userMetadata.team_id}/>;
  if (role === "teacher" && (slug === "classes" || slug === "assignments")) return <TeacherClassesLoader/>;
  if (slug === "settings") return <SettingsPage/>;
  const title = pages[role].find((item) => item.slug === slug)?.label ?? "Dashboard";
  return <GenericPage role={role} title={title}/>;
}
...
function GenericPage({ role, title }: { role: AppRole; title: string }) {
  const descriptions: Record<AppRole, string> = {
    ...
    learner: "Join live classes, complete assignments, review lessons, ask questions, and track progress.",
    guardian: "Follow each child’s classes, progress, payments, teachers, and messages.",
    admin: "Manage users, teacher teams, classes, payments, reports, content, and platform settings.",
  };
